...
sudo apt-get -y update &>> ${CWD}/netflix-proxy.log\
  ...
  && PIP_CONSTRAINT=constraints.txt pip3 install pip --upgrade &>> ${CWD}/netflix-proxy.log\
  && PIP_CONSTRAINT=constraints.txt pip3 install -r requirements.txt &>> ${CWD}/netflix-proxy.log\
  && PIP_CONSTRAINT=constraints.txt pip3 install -r ${CWD}/auth/requirements.txt &>> ${CWD}/netflix-proxy.log
log_action_end_msg $?
